...
/**
 * 기지가 맞은 뒤 이 시간 동안은 **새 예약을 못 받는다** (대전 전용).
 *
 * 오너의 설계 명제: 러시의 값어치는 안개 속에서 모아 온 기습인데,
 * 지키는 쪽이 그 순간 쌓아둔 돈으로 병력을 뽑아내면 형평성이 깨진다.
 *
 * 실측이 뒷받침한다 (tools/ambush.mjs, 4기지 수비 18판): 쌓아둔 돈이
 * 0코면 기습이 94% 통하는데 120코면 50%로 떨어지고, 교전 한 번에
 * 55코어치가 솟아났다.
 *
 * 앞서 시험한 레버는 셋 다 실패했다 — 생산 속도는 큐가 하나뿐인 러시를
 * 더 아프게 때렸고(역방향), 예약 상한은 지출만 줄이고 승률은 안 바꿨으며,
 * "적 코앞 배치 금지"는 전선 전체를 묶어 웅크리는 쪽을 제일 크게 죽였다.
 *
 * 그래서 **맞고 있는 그 기지 하나만** 묶는다. 이미 걸어 둔 예약은 그대로
 * 나온다 — 미리 준비한 것은 나오고 **즉석 대응만** 막힌다. 그게 "예측이
 * 실력이 된다"는 말의 기제다.
 */
export const HURT_PRODUCE_LOCK = 4 * TICK_RATE;
